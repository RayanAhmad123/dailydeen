...
- **Format cadence (user preference)**: two video formats run on a **3:1 rotation — 3 silhouette `/daily-video`s, then 1 Ayah video** (footage + ayah + recitation), repeating. Track position in `state/cadence.json` (`silhouettes_since_ayah`): when asked for "the next video", make a silhouette while the counter is <3 (then increment), and an Ayah video when it hits 3 (then reset). Ayah videos use `scripts/fetch_ayat.py` (once) → `scripts/build_ayah.py` → render `AyahVideo`; both formats publish via PostPeer.
