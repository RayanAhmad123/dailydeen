...
- **Format cadence (user preference)**: **AYAH-ONLY as of 2026-07-07** — every "next video" is an Ayah video (footage + ayah + recitation). The silhouette `/daily-video` format is retired for now (Ayah was by far the best-performing format in the channel analysis). Ayah videos use `scripts/fetch_ayat.py` (once) → `scripts/build_ayah.py` → render `AyahVideo`, published via PostPeer. Long multi-ayah passages (≥25s, e.g. ay_fatiha) get **per-ayah segments** automatically: one ayah on screen at a time, synced to the recitation via `assets/recitations/_parts/` durations, text re-fetched per ayah from the API and verified against the bank. A single long ayah (ay_kursi, 55s) can't be segmented — add a font auto-fit before running it. `state/cadence.json` is `rule: "ayah only"` (the old 3:1 silhouette:ayah rotation and `silhouettes_since_ayah` counter are frozen/unused; restore them only if the user reinstates silhouettes). **Reciter/voice: Yasser Al-Dosari** (`Yasser_Ad-Dussary_128kbps` on everyayah.com), set via `RECITER`/`RECITER_NAME` in `scripts/fetch_ayat.py` (changed from Minshawi 2026-07-07). To change the reciter, edit those two constants, delete `assets/recitations/_parts/*` and `assets/recitations/ay_*.mp3` (else cached audio is reused), then re-run `fetch_ayat.py`. Note: only per-ayah reciters on everyayah.com work (Al-Luhaidan is surah-only there, so unavailable). **Metadata language A/B (2026-09-13)**: the Ayah-era audience is ~75% Arabic-speaking (IQ/DZ/MA/EG/SY/SA) while all metadata was English, so `build_ayah.py` now alternates Arabic-first and English titles/descriptions/captions/hashtags per video (`AYAH_META_LANG=ab|ar|en`; Arabic title = ﴿first words of the ayah﴾ | سورة X: N, basmala skipped, Arabic comment CTA). The choice lands in `state/ayah.json` (`meta_lang`) and `state/uploads.json` (`meta_lang`, `theme`, `category: Ayah`); `analyze_performance.py` scores Ayah videos by theme and metadata language (`by_theme`, `by_meta_lang`, duds <20 views excluded). Decide the winner after ~2 weeks (~7 videos per arm). **Bank top-up** (last: 2026-09-13, +31 → 102 entries, runway to ~2026-10-14): add entries + `TITLES` in `fetch_ayat.py`, run it, plan montages in `content/montages.json` (auto-plan pattern from 2026-09-13 respects the clip safe windows), `build_montages.py <ids>`, frame-check, then re-tar and `gh release upload cloud-assets recitations.tar.gz prerendered.tar.gz --clobber`. **TikTok re-posts**: `scripts/repost_tiktok.py --start <date> <ids>` schedules TikTok-only re-posts of failed legs (same PostPeer media URL + caption) one per day at 00:20 UTC — only after checking the account, since PostPeer marks download timeouts failed even when TikTok published (ay_bounty 2026-09-05 was live).
- **Distribution**: **PostPeer** (`scripts/upload_postpeer.py`, `POSTPEER_API_KEY` in `~/.zshenv`) is the primary path — posts to YouTube + TikTok, publishing PUBLIC directly (it ignores YouTube privacyStatus) with the caption, and avoids the TikTok sandbox draft cap. Schedule with `PP_WHEN=<ISO time>`. **QUOTA SPLIT (2026-08-02, both legs scheduled since 2026-09-02)**: the two platforms have opposite quota windows — PostPeer's TikTok app cap resets at 00:00 UTC, PostPeer's shared YouTube quota (100 uploads/day for ALL their users) at midnight Pacific — and each drains within hours of its reset. So the script no longer posts either leg at run time unless the run happens to fall inside that leg's fresh window: `tt_publish_when()` schedules TikTok for the next 00:05 UTC (immediate only if the run fires before 01:00 UTC) and `yt_publish_when()` schedules YouTube for ~10 min after the next Pacific midnight. The YouTube id is back-filled into `state/uploads.json` by the next run's reconcile pass, and leg failures fail the workflow AFTER the state commit (alert email, pipeline still advances). **TikTok settles ASYNCHRONOUSLY** — PostPeer answers a publishNow create with `success: true, pending` and the real verdict lands ~2 min later, which silently ate 7 TikTok posts (2026-08-27..09-02, all `reached_active_user_cap` at the 04:26 UTC slot). Two guards now: `await_terminal()` polls immediate legs to a terminal status before the run reports them, and the reconcile pass flags any TikTok leg that failed in the last 26h (the scheduled case). Never auto-repost a 'failed' TikTok leg — PostPeer marks download timeouts failed even when TikTok published; check the account first. Account IDs + flow in the PostPeer memory. Every upload registers the YouTube id in `state/uploads.json`; `analyze_performance.py` scores them (retention-weighted, every 4th run explores) and the next run follows `work/strategy.json`. Legacy/fallback: `scripts/upload_youtube.py` (official API, honors privacy — use for a PRIVATE review copy); `scripts/upload_tiktok.py` (old sandbox Content Posting API, drafts-only + per-day cap — superseded by PostPeer).
