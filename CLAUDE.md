...
- **Distribution**: **PostPeer** (`scripts/upload_postpeer.py`, `POSTPEER_API_KEY` in `~/.zshenv`) is the primary path — posts to YouTube + TikTok, publishing PUBLIC directly (it ignores YouTube privacyStatus) with the caption, and avoids the TikTok sandbox draft cap. Schedule with `PP_WHEN=<ISO time>`. **QUOTA SPLIT (2026-08-02)**: the two platforms need opposite posting times — PostPeer's TikTok app cap is only clear right after 00:00 UTC (run fires 04:26 UTC), while PostPeer's shared YouTube quota (100 uploads/day for ALL their users) resets at midnight Pacific and drains within hours. So the script posts TikTok immediately and schedules the YouTube leg via PostPeer for ~10 min after the next Pacific-midnight reset; the YouTube id is back-filled into `state/uploads.json` by the next run's reconcile pass, and leg failures fail the workflow AFTER the state commit (alert email, pipeline still advances). Account IDs + flow in the PostPeer memory. Every upload registers the YouTube id in `state/uploads.json`; `analyze_performance.py` scores them (retention-weighted, every 4th run explores) and the next run follows `work/strategy.json`. Legacy/fallback: `scripts/upload_youtube.py` (official API, honors privacy — use for a PRIVATE review copy); `scripts/upload_tiktok.py` (old sandbox Content Posting API, drafts-only + per-day cap — superseded by PostPeer).
